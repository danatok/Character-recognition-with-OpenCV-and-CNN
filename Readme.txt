...
* video camera frame taken-> grayscale
1. feed into face_detect, rects[] -> contain 4 points of rectangle containing the face that is found. Drawing rectangle on the found area.
1. Calculating the histogram, (but firstly converting into hsv) 
1. When hist of face color is calculated, calculate prob of each pixel belongong to that prob or not. 
2. Face region pixels are set to 0, to leave only the pixels belonging to other things that mostly resembles skin color
